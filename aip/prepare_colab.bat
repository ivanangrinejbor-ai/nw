...
powershell -Command "Compress-Archive -Path '.\code_xml_parser.py', '.\tokenizer.py', '.\training_data\projects.json', '.\train_colab.ipynb', '.\train_colab_transformer.ipynb', '.\requirements.txt' -DestinationPath '.\colab_pack.zip' -Force"
echo Created colab_pack.zip
echo.
echo 1. Upload colab_pack.zip to Google Drive
echo 2. Open https://colab.research.google.com
echo 3. File ^> Upload Notebook ^> train_colab_transformer.ipynb (Transformer) или train_colab.ipynb (LSTM)
echo 4. Mount Drive, распаковать zip, запустить все ячейки
echo 5. Скачать transformer_model.pt, vocab.json, model_metadata.json
