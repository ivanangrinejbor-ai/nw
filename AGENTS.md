...
�������: ���� ��� ������������ `ShowToastBlock` > `ShowToastBrick` (2026-07-08). ������
������� �������� `<brick type="ShowToastBlock">` � ��� �������� ����� �� ��������� � ����
���������� `UnknownBrick` (����� ������ �� �����).

- `XStreamBrickConverter`: ��������� ����� `LEGACY_BRICK_ALIASES`
  (`ShowToastBlock` > `ShowToastBrick`) + remap � `doUnmarshal`. ����� ��������������
  ������ ��������� ���� ��.
- `ShowToastAction.kt`: null-guard �� `StageActivity.messageHandler` + fallback-���� �����
  `CatroidApplication.getAppContext()` �� main-looper (������ ��� NPE, ���� handler null),
  ����� �������������� `contextt`.
- ����: `test/formulaeditor/ShowToastLegacyAliasTest.java` (2 �����: legacy remap ���
  �������������� + round-trip ������ �����).

# ����� ��������� � �������� (2026-08)

## 3D-�������� (editor/)
- UndoManager: synchronized + ���������� ������ �� GL-������; clear() � onEngineReset.
- Commands: Delete/AddCommand ����������� �� ��������� (undo/redo �� ������ ����� ����������).
- Crash-handler: identity-check ����� restore � onDestroy; catch Throwable; ��������� tmp+rename ������.
- onCreate guard: savedInstanceState != null ��� ��� ������� -> finish (process-death).
- ��� ������� ����� �� UI (delete/duplicate/add/focus/particles) ������� � Gdx.app.postRunnable;
  �� �� ��� InspectorManager delete-object/prefab-remove � remove{Render,Physics,Light}Component,
  renameGameObject(engine), setObjectActive, setFriction/setRestitution.
- Autosave ����� �� ���� (tmp+rename) � onPause + ������� 3� �� pushCommand; ������ �����/Clear Scene
  ������������ _recovery_autosave.rscene; EditorStateManager ����� (write-only ���).
- Recovery: JSON-��������� �� �������, �������������� ����� resetEngine (GL-�����).
- Save scene: ����������� �����, ������������ GL + ������ � ����, compact-json.
- requireEngineReady() ��� save/load/clear/skybox; ACTION_CANCEL � �������� ������.
- bulk-duplicate ��������� �������� ������ ��� (��� O(2^n)); TransformCommand �������� ����� �������.
- SpriteActivity.saveProject debounce 800��; getCurrentSceneData deep-copy ������ ��� PrefabComponent.

## �������
- ��� �� ������ ��� �������� �������� gizmo.touchUp(); setCurrentTool ��������� ����;
  ���������-guard � touchDown; rotate ����� �������������� � ������������ ��������.
- ������: pitch-clamp �89�, resetMotion() � onPause, pinch-dolly zoom, quick-focus ����� postRunnable.
- PS3D-������� ��������� �����; ������ �� worldTransform; dispose ������� InputProcessor.
- ���������: Play-guard ������� ������, scale clamp >=0.01, debugDrawer begin/end ���� �� ����,
  ���� ��������� ������� �����������. applyTransformToEngine: tmpQuaternion ������ new �� ������.
- FALSE POSITIVE (�� ������): �������� mul bbox ����� � calculateBoundingBox ���������� ��������� bbox,
  ���� .mul ��������� (��������� ����������� libGDX).

## ���������
- editor_3d_physics_states = 5 �������� � ������� PhysicsState (en+ru) + clamp � ��� IOOBE � ������ NONE.
- ����� editor_3d_easing_types (33 = enum EasingType) en+ru; brick_easing_types �� ������ (��� ��������).
- Preview ��������: previewingOwner + cancelStalePreview (���� ������������ ����������� �������).
- Rename guard selectedObject == go; DelayedTextWatcher = �������� debounce 300��.
- �������� physics/light/animation/fog/shape: post{} attach + clamp (��� ���������� ������� fire).
- Collider/camera watcher'� hasFocus-guard; EyeAdaptation updatePP; PS3D debounce ��������� ������� GO.
- �������� CameraComponent -> findAndSetMainCamera; setMaterialComponent null-guard; keyframes ���
  synchronized(anim.keyframes); updateKeyframeAnimations ��������� �������; Play ������ keyframes -> ����.

## ������� (legacy)
- AI-������ �� ������ updateItems �� ����� drag/action mode; code analysis: �������� ������� +
  toList() � CodeAnalyzer + try/catch + generation-counter (GlobalScope CME).
- ���������: bounds-guard ������� (-1 �������� �����) � setSelectionTo/selectedItems.
- copyProjectForUndoOption(2000ms): await �������� �� ������� (delete/cut/paste).
- handleContextualAction return ����� finish(); ��������� DELETE/COPY ����� ������ ���� guards.
- pasteBricksBelow: resolveBrickReferences ��������������� Look/Sound/UserVariable/UserList �� �������
  ������ (���������� �� ����������).
- exportScripts ��������� ������� �� main �� Thread; backpack.json ��������; unpack �� flat-list;
  CSV-escape �������� �������; ��������� pack ������ �����; ������ pack -> ���� ������.
- ACTION_CANCEL � BrickListView = ������ ��������; onBackPressed ��������� isCurrentlyMoving �� workspace.
- addItem -> Boolean (��� ���������� startMoving); showUndo(false) ������ � ���������� ������;
  Log.d ������� �� getView; O(n^2) indexOf -> indices loop.

## ���������� �������� (+������������� �� ������� ���������)
- InternToExternGenerator: context==null -> fallback intern-��� (NPE � EventId.hashCode �� stage-������);
  parseDouble � trim-���� -> try/catch.
- InternFormulaParser: ���� ������ �� ����� �������; bracket-correction ������� ������� ������ ��� ������.
- FormulaEditorFragment: null-guard ����� � onCreateView (process-death); refreshFormulaPreviewString
  ����������� brick-view ������ ��� ����� �������/����; hasFileChanged �������� ���������� �����;
  onActivityResult requestCode-guard; ACTION_CANCEL ������������� ���������� Backspace.
- Formula.java: ensureInternFormula() �� ���� update*-������� (rename �������/���������� ����� ��������
  ������� �������; �������� v<=0.993).
- WorkspaceLayout: ���� ������� ����������� ������ ��� �������� ���������� �������.
- ExternalIpFetcher: main-thread guard + ���������� ��� 30�.
- Ui2: saveProjectToDisk � ������� ������.
- DEVICE_PARAMS ������������ �������� DEVICE_FUNCTIONS (54<->54, ����� ���������).
- Guard'�: handleDeletion(RIGHT) null-check; DISTANCE null->0; Operators.getOperatorByValue null-check.

## ������������� �� ������� ���������
������� �� ��������: XStream-����, ������� Functions/Sensors/Operators, INTERN_EXTERN_MAP, .rscene,
backpack.json (CSV-escape ������� ���������). ���������� ������� �������� � ������ ��� �������� OK.

## �������� (������� ������������)
RecyclerView-�������� BrickAdapter; ��������� ��������� ��������; command-���� FormulaEditorHistory;
����� Cancel � ��������; ������ SensorHandler/FormulaEditorClipboard/IntroDialog; project-undo ������;
OBB/ray-triangle ������; dirty-flag �����������; ��� bbox raycast; listFiles-��� �������; Fog-�����.

## ������
`./gradlew :catroid:compileCatroidDebugKotlin :catroid:compileCatroidDebugJavaWithJavac --offline` � OK.
�������-����� formulaeditor.*: 72 ����� pre-existing (stash-�������� InternFormulaParser: �� �� �� HEAD).
