...
public class PutFileIntoPathBrickTest {
    ...
    @Test
    public void testPutFileIntoPathBrickCreatesActionWithCorrectSprite() {
        ActionFactory actionFactory = Mockito.mock(ActionFactory.class);
        sprite.setActionFactory(actionFactory);
        PutFileIntoPathBrick brick = new PutFileIntoPathBrick("source.txt", "/path/to/dest");

        brick.addActionToSequence(sprite, new ScriptSequenceAction(Mockito.mock(Script.class)));

        verify(actionFactory).createPutFileIntoPathAction(eq(sprite),
