...
public class JumpingSumoMoveBackwardAction extends TemporalAction {
	...
	protected void update(float percent) {
		...
		try {
			...
			float speedValue = speed == null ? 0f : speed.interpretFloat(scope);
			Log.i(TAG, "Jumping Sumo move backward: duration=" + durationValue
					+ "s, speed=" + speedValue + "% (hardware not available)");
