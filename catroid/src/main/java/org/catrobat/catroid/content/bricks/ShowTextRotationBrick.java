...
public class ShowTextRotationBrick extends UserVariableBrickWithVisualPlacement {
    ...
    public ShowTextRotationBrick() {
        addAllowedBrickField(BrickField.TEXT, R.id.brick_show_text_rotation_edit_text);
        addAllowedBrickField(BrickField.NAME, R.id.brick_show_text_rotation_edit_name);
        addAllowedBrickField(BrickField.FILE, R.id.brick_show_text_rotation_edit_font);
        addAllowedBrickField(BrickField.X_POSITION, R.id.brick_show_text_rotation_edit_x);
        addAllowedBrickField(BrickField.Y_POSITION, R.id.brick_show_text_rotation_edit_y);
        addAllowedBrickField(BrickField.SIZE, R.id.brick_show_text_rotation_edit_size);
        addAllowedBrickField(BrickField.COLOR, R.id.brick_show_text_rotation_edit_color);
        addAllowedBrickField(BrickField.VALUE, R.id.brick_show_text_rotation_edit_rotation);

        showFormulaEditorStrategy = new ShowColorPickerFormulaEditorStrategy();
    }

    public ShowTextRotationBrick(String name, String text, float rotation, String font, int xPosition, int yPosition, double size, String color) {
        this(new Formula(name), new Formula(text), new Formula(font), new Formula(rotation), new Formula(xPosition), new Formula(yPosition), new Formula(size), new Formula(color));
    }

    private ShowTextRotationBrick(Formula name, Formula text, Formula font, Formula rotation, Formula xPosition, Formula yPosition, Formula size, Formula color) {
        this();
    ...
    public void showFormulaEditorToEditFormula(View view) {
        if (view.getId() == R.id.brick_show_text_rotation_edit_color && isValidColorString(getColor())) {
    ...
    public int getViewResource() {
        return R.layout.brick_show_text_rotation;
    }

    @Override
    protected int getSpinnerId() {
        return R.id.brick_show_text_rotation_spinner;
    }
    ...
    public View getView(Context context) {
        ...
        BrickSpinner<AlignmentStyle> spinner =
                new BrickSpinner<>(R.id.brick_show_text_rotation_spinner, view, items);
    ...
    public void addActionToSequence(Sprite sprite, ScriptSequenceAction sequence) {
        sequence.addAction(sprite.getActionFactory().createShowTextRotationAction(sprite,
                sequence, getFormulaWithBrickField(BrickField.NAME), getFormulaWithBrickField(BrickField.TEXT), getFormulaWithBrickField(BrickField.VALUE), getFormulaWithBrickField(BrickField.FILE), getFormulaWithBrickField(BrickField.X_POSITION),
    ...
    public int getXEditTextId() {
        return R.id.brick_show_text_rotation_edit_x;
    }

    @Override
    public int getYEditTextId() {
        return R.id.brick_show_text_rotation_edit_y;
    }
