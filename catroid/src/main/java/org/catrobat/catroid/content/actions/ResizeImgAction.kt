...
class ResizeImgAction() : TemporalAction() {
    ...
    override fun update(percent: Float) {
        ...
        val width = w?.interpretInteger(scope)
        val height = h?.interpretInteger(scope)
        file_f.let{
            val originalBitmap: Bitmap? = ImageProcessingManager.loadBitmapFromFile(it)
            originalBitmap.let { bmp ->
                val resized: Bitmap? = ImageProcessingManager.resizeBitmap(bmp, width, height)
