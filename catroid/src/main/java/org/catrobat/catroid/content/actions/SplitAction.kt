...
class SplitAction() : TemporalAction() {
    private var contextt: Context? = null
    var scope: Scope? = null
    var string: Formula? = null
    var simbol: Formula? = null
    ...
    override fun update(percent: Float) {
        ...
        val str = string?.interpretString(scope) ?: ""
        val simb = simbol?.interpretString(scope) ?: ","
        val massive = splitString(str, simb)

        for(i in massive) {
            userList?.addListItem(i)
        }
    }
