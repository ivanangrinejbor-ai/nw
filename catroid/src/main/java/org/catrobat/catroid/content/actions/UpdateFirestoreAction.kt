...
class UpdateFirestoreAction() : Action() {
    ...
    override fun act(delta: Float): Boolean {
        if (!started) {
            ...
            if (waitForResponse) {
                FirestoreManager.updateDocument(baseStr, pathStr, valueStr) { finished = true }
                return finished
            }
            FirestoreManager.updateDocument(baseStr, pathStr, valueStr)
            return true
        }
        return finished
    }
