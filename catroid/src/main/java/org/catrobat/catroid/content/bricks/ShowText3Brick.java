...
public class ShowText3Brick extends UserVariableBrickWithVisualPlacement {
    ...
    public ShowText3Brick() {
        addAllowedBrickField(BrickField.TEXT, R.id.brick_show_text_edit_text);
        addAllowedBrickField(BrickField.NAME, R.id.brick_show_text_edit_name);
        addAllowedBrickField(BrickField.X_POSITION, R.id.brick_show_text_edit_x);
        addAllowedBrickField(BrickField.Y_POSITION, R.id.brick_show_text_edit_y);
        addAllowedBrickField(BrickField.SIZE, R.id.brick_show_text_edit_size);
        addAllowedBrickField(BrickField.COLOR, R.id.brick_show_text_edit_color);

        showFormulaEditorStrategy = new ShowColorPickerFormulaEditorStrategy();
    }

    public ShowText3Brick(String name, String text, int xPosition, int yPosition, double size, String color) {
        this(new Formula(name), new Formula(text), new Formula(xPosition), new Formula(yPosition), new Formula(size), new Formula(color));
    }

    private ShowText3Brick(Formula name, Formula text, Formula xPosition, Formula yPosition, Formula size, Formula color) {
        this();
    ...
    public void showFormulaEditorToEditFormula(View view) {
        if (view.getId() == R.id.brick_show_text_edit_color && isValidColorString(getColor())) {
    ...
    public int getViewResource() {
        return R.layout.brick_show_text3;
    }

    @Override
    protected int getSpinnerId() {
        return R.id.brick_show_text_spinner;
    }
    ...
    public View getView(Context context) {
        ...
        BrickSpinner<AlignmentStyle> spinner =
                new BrickSpinner<>(R.id.brick_show_text_spinner, view, items);
    ...
    public void addActionToSequence(Sprite sprite, ScriptSequenceAction sequence) {
        sequence.addAction(sprite.getActionFactory().createShowtext3Action(sprite,
                sequence, getFormulaWithBrickField(BrickField.NAME), getFormulaWithBrickField(BrickField.TEXT), getFormulaWithBrickField(BrickField.X_POSITION),
    ...
    public int getXEditTextId() {
        return R.id.brick_show_text_edit_x;
    }

    @Override
    public int getYEditTextId() {
        return R.id.brick_show_text_edit_y;
    }
