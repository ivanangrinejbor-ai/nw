...
class WriteFirestoreAction() : Action() {
    ...
    override fun act(delta: Float): Boolean {
        if (!started) {
            ...
            if (waitForResponse) {
                FirestoreManager.writeDocument(baseStr, pathStr, valueStr) { finished = true }
                return finished
            }
            FirestoreManager.writeDocument(baseStr, pathStr, valueStr)
            return true
        }
        return finished
    }
