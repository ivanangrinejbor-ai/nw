...
class UpdateBaseAction() : Action() {
    ...
    override fun act(delta: Float): Boolean {
        if (!started) {
            ...
            if (waitForResponse) {
                FireBaseManager.updateDatabase(baseStr, keyStr, valueStr) { finished = true }
                return finished
            }
            FireBaseManager.updateDatabase(baseStr, keyStr, valueStr)
            return true
        }
        return finished
    }
